...
- Status: `PROPOSED / APPROACH_A_APPROVED / WRITTEN_SPEC_REVIEW_PENDING`
- Date: 2026-08-24
...
The v0.2 outer request is the v0.1 request plus one required field, `event_timestamp`. It contains:
...
The fixed origin is local civil `2011-01-01 00:00:00` in `America/New_York`. Accepted local dates
are `2011-01-01` through `2100-12-31`. Minute, second, and fractional-second components MUST all be
zero after timezone normalization. The normalized local month, hour, and weekday MUST equal
`mnth`, `hr`, and `weekday`; weekday uses Sunday=`0` through Saturday=`6`. `season`, `holiday`, and
`workingday` remain supplied, validated request facts because they require the dataset's declared
calendar semantics rather than a target-derived lookup.
...
permitted. An invalid request produces no model prediction and cannot enter a quality denominator.
...
The public serving operation remains single-row `POST /v1/predict`. A v0.2 request adapter produces
the candidate's 18 inputs. For the stable comparator, the same outer request is reduced to the
original 11 v0.1 fields and evaluated by the frozen v0.1 feature contract. The adapter split is
descriptor-selected and hash-bound; it MUST NOT be selected by a mutable alias.
...
edges, and invalid/mismatched timestamps. The native feature-vector digest, candidate native
prediction digest, and ONNX Runtime prediction digest MUST recompute under the locked dependency
graph. No H2 row may be used for these vectors.
...
The **final v0.2 candidate artifact** does not exist until one eligible development trial wins,
passes exact replay, is refit on 2011 plus H1, and completes Section 8. None of these three identities
may be conflated.

## 6. Finite candidate protocol

### 6.1 Formal search receipt

Before the first formal development fit, one canonical frozen search receipt MUST be committed. It
binds this spec digest, exact trial table, fold table, ranking rule, dependency lock, source SHA,
dataset/development identities, feature schema, seeds, compute ceiling, quality policy, statistical
code, and an assertion that H2 is sealed. Any byte change creates a new protocol version and cannot
continue the same formal run.
...
coefficient can extend beyond the training maximum while bounded calendar terms model periodicity.
...
- duplicate/missing paired identity;
...
- exactly four additional fits for one winning trial's deterministic fold replay;
...
Only trials that PASS every Section 7 condition enter ranking. The winner is the lexicographic
minimum of:
...
The rank uses only 2011 plus observed H1 development rows. `CTRL-01` is excluded. Exactly one winner
may be selected. If no trial passes, the result is `NO_ELIGIBLE_CANDIDATE`; no final candidate is
trained and H2 remains sealed. A failed winner cannot be replaced after freeze or H2 access.
...
A trial PASSes development only if all conditions hold:

1. all four fold receipts, identities, predictions, and labels are complete, paired, finite,
   non-negative, deterministic, and policy-matched;
...
12. the selected trial's four-fold exact replay reproduces configuration, preprocessing,
    prediction-vector, metric, and receipt digests;
13. all development artifacts and reports have exact SHA-256 identities and no private path or raw
    environment data.
...
Any threshold failure is `FAIL`. Missing, invalid, nondeterministic, statistically insufficient, or
budget-exceeded evidence is `UNKNOWN`. Neither permits final-candidate construction.

## 8. Final training, deployment feasibility, and freeze

### 8.1 Final refit

Only the unique development winner may be refit. The final development interval is exactly
`[2011-01-01 00:00, 2012-07-01 00:00)`, containing 2011 plus observed H1 and no H2.

The winner's preregistered family, hyperparameters, seed, feature subset, preprocessing, and recency
rule are unchanged. A `REC` winner uses its exact trailing window ending at `2012-07-01 00:00`; a
`STAT` or `NL` winner uses the full final development interval. There is no post-selection
calibration unless calibration was already part of that winning trial's exact pipeline. There is no
second candidate.
...
| `T1_TEMPORAL_DEVELOPMENT` | approved implementation plan and complete implementation | one development winner and complete trial evidence | `NO_ELIGIBLE_CANDIDATE`; H2 sealed |
...
1. strict timestamp/timezone/cross-field validation and fail-closed reason codes;
2. exact 18-field feature order/formulas and golden vectors, including DST/leap/year boundaries;
...
7. paired metric/bootstrap frozen vectors, subgroup completeness, and `UNKNOWN` properties;
8. cross-fold stability, no-single-fold-win, deterministic replay, and tie-break tests;
...
11. H2 permission/state/concurrency tests proving one token, consume-before-read, and no reset after
    crash;
12. confirmatory PASS/FAIL/UNKNOWN fixtures and proof that no result can spawn a second candidate;
...
contract, comparator, policy, artifact, and local resource profile.
...
- one development winner and one H2 candidate;
...
`DESIGN_SPEC_COMMITTED / H2_SEALED / OWNER_WRITTEN_SPEC_REVIEW_REQUIRED`
